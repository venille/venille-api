  ...
  implements ICommandHandler<LogPeriodHistoryCommand>
{
  ...
  async execute(command: LogPeriodHistoryCommand) {
    try {
      ...
      // Build desired records grouped by start month/year
      ...
      for (const yearInfo of payload.years) {
        for (const periodInfo of yearInfo.months) {
          const desiredStart = new Date(periodInfo.startDate);
          const desiredEnd = new Date(periodInfo.endDate);
          const key = `${desiredStart.getFullYear()}-${desiredStart.getMonth()}`;
          const list = desiredByMonth.get(key) ?? [];
          list.push({ startDate: desiredStart, endDate: desiredEnd });
          desiredByMonth.set(key, list);
        }
      }

      // Build existing records grouped by start month/year
      const existingByMonth = new Map<string, PeriodTrackerRecord[]>();
      for (const record of periodRecords) {
        const recordStart = new Date(record.startDate);
        const key = `${recordStart.getFullYear()}-${recordStart.getMonth()}`;
        const list = existingByMonth.get(key) ?? [];
        list.push(record);
        existingByMonth.set(key, list);
      }

      // Reconcile per month: exact matches are kept, differences are updated,
      ...
      for (const key of allMonthKeys) {
        const desiredList = (desiredByMonth.get(key) ?? []).sort(
          (a, b) => a.startDate.getTime() - b.startDate.getTime(),
        );
        const existingList = (existingByMonth.get(key) ?? []).sort(
          (a, b) =>
